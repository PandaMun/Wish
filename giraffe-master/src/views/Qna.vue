        ...
        <div>
          <div>
            ...
            <v-dialog persistent v-model="write" max-width="600px">
              ...
              <v-card>
                <v-card-title>
                  <span class="text-h5 font-weight-bold">문의사항 등록</span>
                </v-card-title>
...
export default {
  ...
  methods: {
    registQna() {
      ...
      http.post("/wish/inquiries", qnaInfo).then(({ data }) => {
        console.log(data);
        if (data === "등록이 성공적으로 되었습니다.") this.write = false;
        this.makeList();
        this.title = "";
        this.content = "";
      });
    ...
    makeList() {
      http.get("/wish/inquiries").then(({ data }) => {
        console.log(data);
        this.qnaList = data;
    ...
    modifyQna(i) {
      let url = "wish/inquiries/" + this.qnaList[i].id;
      http
        .patch(url, {
          id: this.qnaList[i].id,
          userId: "ssafy",
